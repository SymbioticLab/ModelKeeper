...
__all__ = [
    ...
    'vgg19_bn', 'vgg19', 'make_layers', 'vgg_zoo'
]
...
def make_layers(cfg, batch_norm=False, k = 3, num_of_class = 10):
    layers = []
    in_channels = 3
    for v in cfg:
        if v == 'M':
            ...
            conv2d = nn.Conv2d(in_channels, v, kernel_size=k, padding=int((k-1)/2))
            if batch_norm:
...
#vggzoo = vgg_zoo()
#print(vggzoo)
#for i in range(len(vggzoo)):
#    for j in range(i+1, len(vggzoo)):
#        assert (vggzoo[i] != vggzoo[j])
#
#print(len(vggzoo))
