...
# AED
from modelkeeper.aed_matcher import AEDMatcher
# Libs for model clustering
from modelkeeper.clustering import k_medoids
from modelkeeper.evictor import mip
from modelkeeper.mapper import MappingOperator

# Call C backend
clib_matcher = ctypes.cdll.LoadLibrary(
    os.path.join(
        os.path.dirname(__file__),
        'backend/bin/matcher.so'))
...
log_path = f'{os.environ["HOME"]}/modelkeeper_log'
...
class MatchingOperator(object):
    ...
    def __init__(self, parent):
        ...
        self.aed_matcher = AEDMatcher(
            AED_PATH, get_onnx_name(
                parent.graph['name'])) if IS_AED else None

    def init_parent_index(self):
        # generate a dict of (nodeID) -> (index into nodelist (and thus
        # matrix))
        ...
        self.meta_data['parent'] = {
            'opts': [
                self.parent.nodes[x]['attr']['op_type'] for x in self.parentidx_order], 'dims': [
                self.parent.nodes[x]['attr']['dims'] for x in self.parentidx_order], 'parents': [
                self.parentPrevIndicesList[x] for x in self.parentidx_order]}
    ...
    def align_child(self, child, read_mapping):
        ...
        if self.aed_matcher is not None:
            self.match_score, self.match_res = self.aed_matcher.query_child(
                get_onnx_name(child.graph['name']))
            return self.match_score

        # call C lib to get mapping
        json_string = self.dump_meta_json()
        ans_json_str = clib_matcher.get_matching_score(ctypes.c_char_p(
            bytes(json_string, encoding="utf-8")), ctypes.c_bool(read_mapping))

        self.match_score, self.match_res = self.parse_json_str(
            ans_json_str, read_mapping)

        logging.info(
            f"{self.parent.graph['name'].split('/')[-1]} align_child {self.child.graph['name'].split('/')[-1]} takes {time.time() - start_time} sec, " +
    ...
    def get_mappings(self):
        ...
        if self.aed_matcher is not None:
            self.match_score, self.match_res = self.aed_matcher.query_child(
                get_onnx_name(self.child.graph['name']))
    ...
    def dump_meta_json(self):
        ...
        self.meta_data['child'] = {
            'opts': [
                self.child.nodes[x]['attr']['op_type'] for x in self.childidx_order], 'dims': [
                self.child.nodes[x]['attr']['dims'] for x in self.childidx_order], 'parents': [
                self.childPrevIndicesList[x] for x in self.childidx_order]}  # ,
    ...
    def parse_json_str(self, ans_json_str, read_mapping):
        ...
        if read_mapping:
            backGrphIdx, backStrIdx = collections.defaultdict(
                list), collections.defaultdict(list)

            # parse information
            for key in ans['backParentIdx']:
                hash_v = key.split('_')
                backGrphIdx[int(hash_v[0]), int(hash_v[1])
                            ] = ans['backParentIdx'][key]

            for key in ans['backChildIdx']:
                hash_v = key.split('_')
                backStrIdx[int(hash_v[0]), int(hash_v[1])
                           ] = ans['backChildIdx'][key]
    ...
    def backtrack(self, backGrphIdx, backStrIdx):
        ...
        while len(que) != 0:
            besti, bestj = que.pop()
            curstridx, curnodeidx = self.childidx_order[bestj - \
                1], self.parentidx_order[besti - 1]

            # last step to (besti, bestj)
            nextis, nextjs = backGrphIdx[besti,
                                         bestj], backStrIdx[besti, bestj]

            name_aligned = True
            if curstridx is not None and curnodeidx is not None:
                name_aligned = self.child.nodes[curstridx]['attr'][
                    'op_type'] == self.parent.nodes[curnodeidx]['attr']['op_type']

            # we pad a gap
            name_aligned = name_aligned and (
                bestj not in nextjs and besti not in nextis)
            ...
            for nexti, nextj in zip(nextis, nextjs):
                if not(
                        nexti == 0 and nextj == 0) and (
                        nexti,
                        nextj) not in que_set:
...
def mapping_func(
        ...
        child,
        read_mapping=False,
        return_child_name=False):
    parent_name = parent_opt.parent.graph['name']

    try:
        score = parent_opt.get_matching_score(
            child=child, read_mapping=read_mapping)
    except Exception as e:
        logging.error(
            f"Mapping {parent_name} to {child.graph['name']} failed as {e}")
        score = float('-inf')
...
class ModelKeeper(object):
    ...
    def init_model_zoo(self, zoo_path):
        if os.path.exists(zoo_path):
            model_paths = [
                os.path.join(
                    zoo_path,
                    x) for x in os.listdir(zoo_path) if x.endswith('.onnx')]
    ...
    def add_to_zoo(self, model_paths):
        ...
        if self.args.zscore_disable is False:
            model_accuracies = [p.parent.graph['accuracy']
                                for p in self.model_zoo.values()]
            ...
            model_accuracies = np.array(
                model_accuracies + new_model_accuracies)
            ...
            for m in existing_zoo:
                if self.model_zoo[m].parent.graph['accuracy'] < accuracy_mean - \
                        self.outlier_factor * accuracy_std:
        ...
        for model_path in decent_models:
            ...
            if model_path in self.model_zoo:
                ...
                try:
                    model_graph, model_weight = self.load_model_meta(
                        model_path)
                    ...
                    # with self.zoo_lock:
                    self.model_zoo[model_path] = MatchingOperator(
                        parent=model_graph)
        ...
        if is_update and self.zoo_storage > self.zoo_capacity:
            ...
            util_before_evict = int(
                sum([self.model_zoo[m].model_value for m in self.model_zoo]))

            logging.info(
                f"Evict model {[(self.model_zoo[m].model_weight, m) for m in models_to_evict]}")

            self.remove_from_zoo(models_to_evict)

            logging.info(
                f"Zoo storage ({size_before_evict} MB, {util_before_evict}) updates to ({int(self.zoo_storage)} MB," +
    ...
    def remove_from_zoo(self, model_paths):
        ...
        for model_path in model_paths:
            if model_path in self.model_zoo:
                ...
                logging.warning(
                    f"Fail to remove {model_path} from zoo, as it does not exist")
            ...
            for model_b in models_for_clustering:
                if model_a not in evicted_nodes and model_b not in evicted_nodes:
                    if model_a != model_b and self.distance[model_a][model_b] < self.args.neigh_threshold and \
                        ...
                        if self.model_zoo[model_a].parent.graph['accuracy'] != - \
                                1 and self.model_zoo[model_b].parent.graph['accuracy'] != -1:
    ...
    def update_model_clusters(
        ...
        def update_cluster_offline():
            ...
            updating_pairs = [
                (i, j) for i in current_zoo_models for j in current_zoo_models if (
                    i not in self.distance or j not in self.distance[i]) and i != j]
            ...
            for (model_a, model_b) in updating_pairs:
                results.append(
                    pool.apply_async(
                        mapping_func,
                        (self.model_zoo[model_a],
                         self.model_zoo[model_b].parent,
                            False,
                            True)))
            ...
            # 2. Evict neighbors if similarity > threshold (0.95 default) and
            # neigh_acc < node_acc
            ...
            logging.info(
                f"Cluster into {num_of_clusters} clusters, max_diameter: {diameter}")
    ...
    def load_model_meta(self, meta_file='sample__accuracy.onnx'):
        ...
        nodes = [
            n for n in onnx_model.graph.node if n.op_type not in self.skip_opts]
        graph = nx.DiGraph(
            name=meta_file,
        ...
        for idx, node in enumerate(nodes):
            ...
            layer_name = None if not trainable_weights else '.'.join(
                trainable_weights.split('.')[:-1])

            attr = {
                'dims': [] if not trainable_weights else node_shapes[trainable_weights],
                'op_type': self.get_op_type(node.op_type, layer_name),
                # if node.name else
                # str(node.op_type)+str(opt_dir[node.op_type]),
                'name': node.name,
    ...
    def query_scores(self, parents, child, threads=20, timeout=600):
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=threads) as executor:
            try:
                for model, score in executor.map(
                        mapping_func, parents, repeat(child), timeout=timeout):
                    scores.append((model, score))
            except Exception as e:
                for pid, process in executor._processes.items():
                    process.terminate()
                logging.warning(
                    f"Query scores for {child.graph['name']} fails, as: {e}")
    ...
    def query_best_mapping(self, child, blacklist=set(),
        ...
        if len(medoids) > 0:
            medoid_dist = self.query_scores(
                [self.model_zoo[p] for p in medoids], child, self.args.num_of_processes)
        ...
        for top_i in range(len(medoid_dist)):
            ...
            for cluster_id in self.model_clusters:
                if cluster_id.kernel == cluster:
                    search_models += [self.model_zoo[e]
                                      for e in self.model_clusters[top_i].elements if e != cluster]
                    break
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=threads) as executor:
            # for model, score in executor.map(self.mapping_func,
            # search_models, timeout=timeout):
            try:
                for model, score in executor.map(
                        mapping_func, search_models, repeat(child), timeout=timeout):
                    if score > best_score:
        ...
        if parent_path is not None and return_weight:
            with concurrent.futures.ProcessPoolExecutor(max_workers=1) as executor:
                try:
                    for p, m, s in executor.map(mapping_func, [self.model_zoo[parent_path]], [
                                                child], [True], timeout=timeout):
        ...
        if parent is not None:
            logging.info(
                "{} find best mappings {} (score: {}) takes {:.2f} sec\n\n".format(
                    child.graph['name'], parent.graph['name'], round(
                        best_score, 4), time.time() - start_time))
        else:
            logging.info(
                "{} does not find best mapping, takes {:.2f} sec\n\n".format(
                    child.graph['name'],
    ...
    def bucketing_selection(self, results, _max, _min):
        ...
        bucket_boundary = [
            i *
            1. /
            self.bucket_interval for i in range(
        ...
        for (p, s) in results:
            ...
            bucket_id = min(
                bisect.bisect_left(
                    bucket_boundary,
                    norm_score),
                self.bucket_interval - 1)
            buckets[bucket_id].append(
                (p, s, self.model_zoo[p].parent.graph['accuracy']))

        logging.info(
            f"bucketing information:\n{buckets}\n, max {_max}, min {_min}")
    ...
    def get_best_mapping(
        ...
        parent_models = [
            model for model in self.model_zoo.keys() if model not in blacklist]
        results = self.query_scores([self.model_zoo[p] for p in parent_models] + [
                                    MatchingOperator(child)], child, self.args.num_of_processes)
        ...
        if self.bucket_selection:
            parent_path, best_score = self.bucketing_selection(
                matching_results, self_score, worst_score)
        ...
        if parent is not None:
            logging.info(
                "{} find best mappings {} (score: {}, {}) takes {:.2f} sec\n\n".format(
                    child.graph['name'], parent.graph['name'], round(
                        best_score, 4), round(
                        best_score / self_score, 4), time.time() - start_time))
        else:
            logging.info(
                "{} does not find best mapping, takes {:.2f} sec\n\n".format(
                    child.graph['name'],
    ...
    def warm_weights(self, parent, child, mappings):
        ...
        mapper = MappingOperator(
            parent,
            child,
            mappings,
            breakdown=self.DISABLE_TANSFORMER)
        mapper.cascading_mapping()
        mapper.pad_mapping()
        weights, num_of_matched, layer_mappings = mapper.get_mapping_weights()

        logging.info(
            "Mapped {} layers to the child model ({} layers), parent {} layers".format(
                num_of_matched,
                child.graph['num_tensors'],
    ...
    def map_for_model(
        ...
        onnx_model_name = os.path.join(
            self.args.zoo_query_path, str(
                self.current_mapping_id) + ".onnx_temp")
        if hidden is None:
            torch.onnx.export(
                child_model,
                dummy_input,
        ...
        parent, mappings, best_score = self.get_best_mapping(
            child, blacklist, model_name)
        ...
        if mappings is not None and len(mappings) > 0:
            ...
            # print(mappings, f"# of parent nodes:
            # {parent.graph['num_tensors']}, # of child nodes:
            # {child.graph['num_tensors']}, # of mapped pairs:
            # {len(mappings)}")
        if parent is not None:
            weights, num_of_matched, layer_mappings = self.warm_weights(
                parent, child, mappings)
            if num_of_matched > THRESHOLD * \
                    max(parent.graph['num_tensors'], child.graph['num_tensors']):
            ...
            logging.info(
                f"Querying model {child.graph['name']} completes with meta: {meta_data}")
    ...
    def map_for_onnx(self, child_onnx_path, blacklist=set(), model_name=None):
        ...
        if len(self.model_zoo) > self.mode_threshold:
            parent, mappings, best_score = self.query_best_mapping(
                child, blacklist, model_name)
        else:
            parent, mappings, best_score = self.get_best_mapping(
                child, blacklist, model_name)
    ...
    def export_query_res(self, model_name, weights, meta_data):
        ...
        ans = os.system(
            f"mv {export_path} {export_path.replace('.onnx', '.out')}")

    def check_register_models(self):
        new_models = [
            x for x in os.listdir(
                self.args.zoo_register_path) if x.endswith('.onnx')]
        if len(new_models) > 0:
            for m in new_models:
                os.system(
                    f'mv {os.path.join(self.args.zoo_register_path, m)} {self.args.zoo_path}')

            self.add_to_zoo([os.path.join(self.args.zoo_path, m)
                             for m in new_models])
    ...
    def check_pending_request(self):
        request_models = [
            x for x in os.listdir(
                self.args.zoo_query_path) if x.endswith('.onnx')]
        if len(request_models) > 0:
            for m in request_models:
                ...
                weights, meta_data = self.map_for_onnx(
    ...
    def start(self):
        ...
        while True:
            ...
            if time.time() - last_heartbeat > 30:
                logging.info(
                    f"ModelKeeper has been running {int(time.time() - start_time)} sec ...")
