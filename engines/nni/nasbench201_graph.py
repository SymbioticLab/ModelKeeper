...
class NasBench201TrainingModule(PL.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        #     torch.save(self.model.state_dict(), os.path.join("/users/Yinwei/", model_name+"_start_weight"))  
        #     self.flag = True
        x, y = batch
        y_hat = self(x)
...
def _multi_trial_test(epochs, batch_size, port, benchmark, data):
    ...
    strategy = RegularizedEvolution(optimize_mode='maximize', population_size=200, sample_size=50, cycles=1000, mutation_prob=0.1, on_failure='ignore')
    # dummy_input = torch.rand(8, 3, 32, 32)
    model = NasBench201()
    ...
    exp_config.trial_concurrency = 14
