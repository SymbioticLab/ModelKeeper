...
def load_data(file):
    ...
    for key in keys:
        try:
            ...
            for m in model_pairs:
                model_pairs[m][key] = min(max(1e-5, (model_pairs[m][key] - _min)/(_max-_min)), 0.9999)
...
def get_dist_matrix(model_pairs):
    ...
    for idx, model in enumerate(models):
        ...
        for i in range(num_models):
            ...
            dist_matrix[idx][i] = max(1e-3, 1.0-(model_pairs[model][child_model]+model_pairs[child_model][model])/2.0)
...
def dryrun(file='imagenet-cv-zoo', factor=1):
    ...
    return {'Quality': ans, 'Sim': [sum(x)/len(x) for x in current_sim], 'Matches': sum(tiktak)/len(tiktak)/len(tiktak)*100.0, 'k': k}
    # print('Quality: ', ans)
