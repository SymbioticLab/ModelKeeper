...
def get_data_loaders(train_bz, test_bz, tokenizer=None, model_name=None, interest_args=None):
    ...
    if args.data == 'cifar10':
        ...
        train_dataset = datasets.ImageNet(os.path.join(args.dataset, 'ImageNet'), split='train', download=False, transform=train_transform)
        test_dataset = datasets.ImageNet(os.path.join(args.dataset, 'ImageNet'), split='val', download=False, transform=test_transform)
...
class TrainModel(tune.Trainable):
    ...
    def setup(self, config):
        ...
        num_labels = {'cifar10': 10, "cifar100": 100, "ImageNet16-120": 120, "ImageNet": 1000}
        ...
        self.batch_size = config['config'].get('batch_size', args.batch_size)

        if self.task == "nlp_nwp":
            pass
        else:
            self.train_loader, self.test_loader, _ = \
                get_data_loaders(self.batch_size, args.test_batch_size, self.tokenizer, self.model_name)
        arrival_time = config['config'].get('arrival', 0)

        self.logger.info(f"Setup for model {self.model_name} ..., supposed {arrival_time}, batch size {self.batch_size}")
        ...
        if self.task == "nlp_cls":
            ...
            self.scheduler = optim.lr_scheduler.CosineAnnealingLR(self.optimizer, T_max=args.epochs, eta_min=0)
            #optim.lr_scheduler.ReduceLROnPlateau(self.optimizer, 'max', patience=4,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--trace', type=str, default=f'{os.environ["HOME"]}/experiment/ModelKeeper/ray_tune/workloads/torchcv_zenus.csv')
    ...
    if args.task == "torchcv" or args.task == "ensemble":
        ...
        if args.task == "torchcv":
            temp_conf = workload.to_dict(orient='records')
